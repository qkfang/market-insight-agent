...
public sealed class InsightGenerationAgent : BaseAgent
{
    private const string AgentInstructions = """
        ...
        1. Call list_market_research_history with the market name (if specified) to retrieve all weekly research snapshots for that market, ordered oldest to newest. Look back as many weeks as available — aim for at least 6 weeks of history.
        2. If list_market_research_history returns no results, call get_latest_research as a fallback.
        3. Call read_news_analysis (or list_news_analysis) to gather the latest individual news analysis articles relevant to the market.
